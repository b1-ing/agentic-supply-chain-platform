...
def main():
    ...
    segments_sorted = sorted(segments, key=lambda seg: seg["traffic_level"], reverse=True)

    bands = {"free (0)": 0, "light (0-0.15)": 0, "mid (0.15-0.40)": 0,
             "heavy (0.40-0.80)": 0, "max (0.80-1.0)": 0, "closed": 0}
    ...
    for i, seg in enumerate(segments_sorted[:20], 1):
        ...
        print(f"  {i:>2}. [{flag}] traffic_level={seg['traffic_level']:.3f}  center={loc}")
