...
def serialize_order(order) -> IncomingOrder:
    ...
    return IncomingOrder(
        ...
        earliest_pickup=(
            str(order.earliest_pickup)
            if order.earliest_pickup is not None
            else None
        ),

        latest_pickup=(
            str(order.latest_pickup)
            if order.latest_pickup is not None
            else None
        ),
        ...
        latest_delivery=(
            str(order.latest_delivery)
            if order.latest_delivery is not None
            else None
        ),
...
def get_unserviceable_orders(
    ...
    return [
        serialize_order(order)
        for order in world.unserviceable_orders
    ]
